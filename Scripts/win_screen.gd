...
func _process(delta: float) -> void:
	...
	if t >= 1.0:
		...
		coins_label.text = "Coins Collected: " + str(coins_per) + "% (" + str(coins) + "/" + str(Utils.total_coins) + ")"

		var damned: int = Utils.npcs_damned
		var damned_per: float = float(damned) / float(Utils.total_npcs) * 100
		npcs_label.text = "NPCs Damned: " + str(damned_per) + "% (" + str(damned) + "/" + str(Utils.total_npcs) + ")"
